...
class SLLQueue {
    ...
    interleaveQueue(){
        var midpt = Math.ceil(this.howMany()/2)
        var tempQueue = new SLLQueue();
        for(var i=1; i<= midpt;i++){
            tempQueue.enterQueue(this.leaveQueue())
        }

        var length = tempQueue.howMany()
        for(var j=1; j<=length; j++){
            tempQueue.enterQueue(tempQueue.leaveQueue())
            if(this.head != null){
                tempQueue.enterQueue(this.leaveQueue())   
            }                     
        }
        return tempQueue.display()
    }
}
